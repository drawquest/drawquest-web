...
class GraphAPI(object):
    ...
    def request(self, path, args=None, post_args=None, domain='graph', post_data=None):
        ...
        if post_data is not None and post_args is not None:
            raise ValueError("Cannot specify both post_data and post_args.")
        ...
        for n in range(4):
            try:
                #file = urllib2.urlopen("https://" + domain + ".facebook.com/" + path + "?" +
                #                       urllib.urlencode(args), post_data, timeout=timeout)
                ...
                try:
                    ...
                    file.close()
            except (IOError, ValueError, urllib2.URLError, urllib2.HTTPError, socket.timeout) as e:
                last_exception = e
                time.sleep(n / 20.0) # Backoff a little and try again
                timeout += n * 4
            else:
                break
        else:
...
class GraphAPIError(Exception):
    def __init__(self, type, message, error_code=None):
