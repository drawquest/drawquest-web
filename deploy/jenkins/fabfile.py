...
env.roledefs.update({
    ...
    'drawquest_web': lambda: set(group('Drawquest_web') + group('Drawquest_web_admin') + group('Drawquest_search')),
    'search': functools.partial(group, 'Drawquest_search'),
    'drawquest_staging_web': lambda: set(group('Drawquest_staging_web')),
...
def update_and_build_web(commit='HEAD'):
    update(commit, branch='canv_dot_as')
    with cd('/var/canvas/website'):
        run('python manage.py compress')

@roles('drawquest_web')
def update_drawquest_web(commit='HEAD'):
    update(commit)

@roles('drawquest_staging_web')
def update_drawquest_staging_web(commit='HEAD'):
    update(commit)

@roles('search')
def update_search_index(age=None):
    with cd('/var/canvas/website'):
        cmd = 'DJANGO_SETTINGS_MODULE=settings_drawquest python manage.py update_index --remove'
        if age is not None:
            cmd += ' --age={}'.format(age)
        run(cmd)
...
def nginx():
    with cd('/var/canvas/website'):
        if env.host_string in env.roledefs['drawquest_web']() or env.host_string in env.roledefs['drawquest_staging_web']():
...
@roles('drawquest_staging_web')
def reconfig_drawquest_staging_web():
    nginx()
...
@roles('drawquest_web')
def hard_restart_drawquest_gunicorn():
    with cd('/var/canvas/website'):
...
def pip_install_requirements():
    with cd('/var/canvas'):
        ...
        sudo('nice -n 10 pip install --download-cache=/tmp -r requirements/distribute.txt --no-index')
...
def _backup_redis(project_name):
    with cd('/var/redis'):
        ...
        sudo('s3cmd put {} s3://{}-redis-backup/ --no-progress --multipart-chunk-size-mb=200'.format(fname, project_name))
