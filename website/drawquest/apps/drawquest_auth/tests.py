...
class TestSessionStore(CanvasTestCase):
    def after_setUp(self):
        self.session_dict = {'experiments': {'null_hypothesis': 'experimental'}, '_auth_user_id': 3L, '_auth_user_backend': 'drawquest.apps.drawquest_auth.backends.DrawquestAuthBackend'}

    def test_handling_borked_session(self):
        new_style_data = DjangoDbSessionStore().encode(self.session_dict)
        self.assertEqual(self.session_dict, SessionStore().decode(new_style_data))

    def test_handling_old_style_session(self):
        old_style_data = SessionStore().encode(self.session_dict)
        self.assertEqual(self.session_dict, SessionStore().decode(old_style_data))
...
class TestApi(CanvasTestCase):
    ...
    def test_signup_for_login(self):
        email = 'bob.dole@hotmail.com'
        for _ in range(2):
            resp = self._signup(USERNAME, PASSWORD, email)
            self.assertAPISuccess(resp)

        resp = self._signup(USERNAME, PASSWORD, 'wrong_email@whatever.com')
        self.assertAPISuccess(resp)

        resp = self._signup(USERNAME, PASSWORD, '')
        self.assertAPISuccess(resp)

        self.assertEqual(User.objects.get(username=USERNAME).email, email)
