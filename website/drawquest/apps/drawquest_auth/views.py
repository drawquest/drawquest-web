...
from drawquest.apps.drawquest_auth.forms import PasswordResetForm
...
def deauthorize_facebook_user(request):
    ...
    fb_user = get_object_or_404(FacebookUser, fb_uid=int(fb_uid))

    user = fb_user.user
    if user is not None:
        try:
            user.kv.facebook_privacy.delete()
        except KeyError:
            pass
