...
from canvas.models import UserInfo, Category, Comment, Content, ContentUrlMapping, Visibility
...
class FakeRequest(HttpRequest):
    ...
    @property
    def body(self):
        return getattr(self, '_body', getattr(super(FakeRequest, self), 'body', None))
...
class CanvasTestCase(django.test.TestCase):
    ...
    @classmethod
    def _http_verb(cls, verb, path, client=None, data=None, user=None, password=None, https=False, user_agent=None, **kwargs):
        data = data or {}
    ...
    @classmethod
    def _api_call(cls, path, data=None, client=None, user=None, password=None, method="post", user_agent=None):
        data = data or {}
    ...
    def api_post(cls, *args, **kwargs):
        if kwargs.get('api_version') is not None and settings.PROJECT == 'drawquest':
            kwargs['user_agent'] = 'DrawQuest/{}'.format(kwargs['api_version'])
