...
class EmailChannel(CommunicationsChannel):
    ...
    def enabled_for_recipient_action(cls, action, recipient, pending_notification=None, *args, **kwargs):
        ...
        flag = flag and User.validate_email(recipient.email)

        if settings.PROJECT == 'drawquest':
            from drawquest.apps.bounces.models import SuppressedEmail
            flag = flag and not SuppressedEmail.objects.filter(email=recipient.email).exists()
    ...
    def enabled_for_actor_action(cls, action, actor, *args, **kwargs):
        flag = (super(EmailChannel, cls).enabled_for_actor_action(action, actor, *args, **kwargs)
                and actor.email and User.validate_email(actor.email))
    ...
    def make_message(self, notification, force=False):
        ...
        # Render it.
        subject = render_jinja_to_string(subject_template, context).strip()
        try:
            text = render_jinja_to_string(text_template, context)
    ...
    def make_action_unsubscribe_link(self, action, recipient):
        if settings.PROJECT == 'drawquest':
            ...
            return 'http://{}/unsubscribe?'.format(settings.DOMAIN) + urlencode({
                'action': action,
                'token': util.token(recipient.email),
