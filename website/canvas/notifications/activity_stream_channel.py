...
class ActivityStreamChannel(CommunicationsChannel):
    ...
    def _action_featured_in_explore(self, notification):
        from drawquest.activities import FeaturedInExploreActivity
        return FeaturedInExploreActivity.from_comment(notification.actor, notification.comment)

    def _action_facebook_friend_joined(self, notification):
        from drawquest.activities import FacebookFriendJoinedActivity
        return FacebookFriendJoinedActivity.from_actor(notification.actor)

    def _action_twitter_friend_joined(self, notification):
        from drawquest.activities import TwitterFriendJoinedActivity
        return TwitterFriendJoinedActivity.from_actor(notification.actor)
    ...
    def deliver(self, notification):
        ...
        if not recipient.redis.activity_stream.valid_activity_type(recipient, activity):
            return

        recipient.redis.activity_stream.push(activity)
        recipient.redis.iphone_activity_stream.push(activity)
