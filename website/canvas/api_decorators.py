...
def api_response_wrapper(public_jsonp=False):
    return public_jsonp_service if public_jsonp else json_service

def inner_api(url_decorator, force_csrf_exempt=True):
    def api(url, public_jsonp=False, skip_javascript=False, async=True, csrf_exempt=False):
        ...
        """
        response_wrapper = api_response_wrapper(public_jsonp=public_jsonp)

        csrf_wrapper = django.views.decorators.csrf.csrf_exempt if (csrf_exempt or force_csrf_exempt or settings.API_CSRF_EXEMPT) else lambda f: f
