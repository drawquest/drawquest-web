...
papertrail = logging.getLogger('papertrail')
...
def placeholder(self, field, val):
    if field is None:
        ...
        return val
    elif isinstance(val, Now):
        return val.as_sql(None, self.connection)[0]
...
def as_sql(self):
    ...
    if can_bulk:
        ...
        placeholders = [
            [placeholder(self, field, v) for field, v in zip(fields, val)]
            for val in values
        ]
        # Oracle Spatial needs to remove some values due to #10888
        params = self.connection.ops.modify_insert_params(placeholders, params)
    if self.return_id and self.connection.features.can_return_id_from_insert:
        #import sys; print >> sys.stderr, 'OHHH?'
        params = params[0]
    ...
    if can_bulk:
        #import sys; print >> sys.stderr, 'HUH?'
        result.append(self.connection.ops.bulk_insert_sql(fields, len(values)))
        return [(" ".join(result), tuple([v for val in values for v in val]))]
    else:
        params = [param for param in params if not isinstance(param, Now)]
        _params = []
        for param in params:
            _vals = []
            for val in param:
                if isinstance(val, Now):
                    pass #_vals.append(None)
                else:
                    _vals.append(val)
            _params.append(_vals)
        params = _params
        #import sys; print >> sys.stderr, 'WHAT', placeholders, params
        return [
...
def token(msg):
    ...
    return hmac.new(settings.SECRET_KEY, msg=unicode(msg).encode('utf8')).hexdigest()
...
def make_object(**kwargs):
    class ShortcutObject(object):
        def __init__(self, items):
            for k, v in items:
                setattr(self, k, v)

    return ShortcutObject(kwargs.items())
