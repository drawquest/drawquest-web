...
class Command(NoArgsCommand):
    ...
    def _ipython_embed(self):
        from IPython import embed

        import canvas.models
        import canvas.redis_models
        import drawquest.apps.drawquest_auth.models

        ns = {}
        ns.update(canvas.models.__dict__)
        ns.update(canvas.redis_models.__dict__)
        ns.update(drawquest.apps.drawquest_auth.models.__dict__)

        embed(user_ns=ns)

    def ipython(self):
        """Start any version of IPython"""
        for ip in (self._ipython_embed, self._ipython, self._ipython_pre_100, self._ipython_pre_011):
            try:
                ip()
            except ImportError:
                pass
