...
from configuration import Config, PRODUCTION, STAGING, TESTING
...
def twisted_enabled():
    return options.project == 'canvas' or (options.project == 'drawquest' and (STAGING or PRODUCTION))
...
def generate_nginx_conf(name, context=None):
    ...
    context = context or {}
    return template.render(production=PRODUCTION, staging=STAGING, **context)
...
def gunicorn():
    ...
    if pid:
        ...
        # strace -fv /usr/local/bin/gunicorn_django -c settings_drawquest_gunicorn.py settings_drawquest.py
