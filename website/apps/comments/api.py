...
@api('validate_post')
def validate_comment(request, parent_comment=None, replied_comment=None,
                     reply_content=None, category=None):
...
@api('post')
@require_user
def post_comment(request, anonymous=False, parent_comment=None, replied_comment=None,
                 reply_content=None, category=None, fact_metadata={},
                 title=None, tags=[]):
...
@api('flag')
@require_user
def flag_comment(request, comment_id=None, quest_id=None):
    comment_id = comment_id or quest_id
    ...
    try:
        flag_count = comment.details().flag_counts[0]
    except KeyError:
        flag_count = 0

    auto_moderated = False

    if knobs.AUTO_DISABLE_FLAGGED_COMMENTS_THRESHOLD is not None:
        if (not comment.judged
                and flag_count == knobs.AUTO_DISABLE_FLAGGED_COMMENTS_THRESHOLD[comment.author.userinfo.trusted]):
            comment.visibility = models.Visibility.DISABLED
            comment.save()
            comment.visibility_changed()
            auto_moderated = True

    if (not auto_moderated
            and flag_count >= knobs.AUTO_CURATE_FLAGGED_COMMENTS_THRESHOLD[comment.author.userinfo.trusted]
            and not comment.judged):
        comment.visibility = models.Visibility.CURATED
        comment.save()
        comment.visibility_changed()
        auto_moderated = True
    ...
    return {'flag_counts': comment.details().flag_counts, 'flag_id': getattr(flag, 'id', None)}
