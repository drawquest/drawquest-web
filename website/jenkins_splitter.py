...
def get_previous():
    ...
    aggregatedReport = subprocess.check_output(["curl", "-k", "https://canvas:fakepassword@jenkins.example.com/job/run_tests_in_parallel/lastStableBuild/aggregatedTestReport/"])
    downstreamReports = re.findall('"([^"]+/testReport/)"', aggregatedReport)

    for report in downstreamReports:
        results = json.loads(subprocess.check_output(["curl", "-k", "https://canvas:fakepassword@jenkins.example.com/%s/api/json" % report]))

        for result in results['suites']:
            for case in result['cases']:
                filename = '.'.join(case['className'].split('website.')[-1].split('.')[:-1])
